...
{{--        </div>--}}
        @if(App\Models\Project_Image::where('project_id',$project_id)->count())
            <div class="row row-cols-lg-4 row-cols-md-2">
                @foreach(App\Models\Project_Image::where('project_id',$project_id)->get() as $image )
                    <div class="col my-1">

                        <img src="{{asset('assets/images/projects/'.$image->image)}}" alt="{{$name_en}}" width="100" height="100">
                    </div>
                @endforeach
