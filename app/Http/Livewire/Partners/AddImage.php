...
protected $listeners=[
    ...
    public function add_image($project){
        ...
        isset(Partner::find($this->project_id)->images()->first()->image)? $this->image_org=Partner::find($this->project_id)->images()->first()->image:"";
    }
    public function add(){
        $this->validate([
...
//            $data['image']=$filename;
        ...
        if(isset($filename)){
            Partner::find($this->project_id)->images()->create([
                'image' =>$filename
            ]);}
